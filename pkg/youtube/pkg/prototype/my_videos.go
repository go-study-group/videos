...
func searchListMine(service *youtube.Service, part string, maxResults int64, forMine bool, q string, typeArgument string) {
	call := service.Search.List(part)
	if maxResults != 0 {
		call = call.MaxResults(maxResults)
	}
	if forMine {
		call = call.ForMine(forMine)
	}
	if q != "" {
		call = call.Q(q)
	}
...
func main() {
	...
	searchListMine(service, "snippet", 25, true, "", "video")
}
